[⬅ Sumário](../README.md) · Capítulo 18 de 19

# 18. Referências Oficiais AWS

Use estas fontes primárias para aprofundar qualquer tema tratado neste
ebook — são as mesmas referências recomendadas no guia oficial de estudo
do exame.

## Certificação

- [AWS Certified Solutions Architect – Associate (SAA-C03) — página oficial](https://aws.amazon.com/certification/certified-solutions-architect-associate/)
- [Exam Guide (PDF oficial com os domínios cobrados e seus pesos)](https://d1.awsstatic.com/training-and-certification/docs-sa-assoc/AWS-Certified-Solutions-Architect-Associate_Exam-Guide.pdf)
- [AWS Well-Architected Framework](https://aws.amazon.com/architecture/well-architected/)
- [AWS Well-Architected — Whitepapers por pilar](https://docs.aws.amazon.com/wellarchitected/latest/framework/welcome.html)

## Documentação por serviço

- [Amazon EC2 — User Guide](https://docs.aws.amazon.com/ec2/)
- [Amazon ECS — Developer Guide](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/)
- [AWS Fargate](https://docs.aws.amazon.com/AmazonECS/latest/userguide/AWS_Fargate.html)
- [AWS Lambda — Developer Guide](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html)
- [Amazon VPC — User Guide](https://docs.aws.amazon.com/vpc/latest/userguide/)
- [Amazon S3 — User Guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/)
- [Amazon RDS — User Guide](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/)
- [Amazon DynamoDB — Developer Guide](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/)
- [Amazon ElastiCache for Redis — User Guide](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/)
- [Amazon SQS — Developer Guide](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/)
- [Amazon SNS — Developer Guide](https://docs.aws.amazon.com/sns/latest/dg/)
- [Amazon SES — Developer Guide](https://docs.aws.amazon.com/ses/latest/dg/)
- [AWS Secrets Manager — User Guide](https://docs.aws.amazon.com/secretsmanager/latest/userguide/)
- [AWS Systems Manager Parameter Store — User Guide](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-parameter-store.html)
- [Amazon CloudWatch — User Guide](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/)
- [AWS IAM — User Guide](https://docs.aws.amazon.com/IAM/latest/UserGuide/)
- [Amazon Cognito — Developer Guide](https://docs.aws.amazon.com/cognito/latest/developerguide/)

## Custos e FinOps

- [AWS Pricing Calculator](https://calculator.aws)
- [AWS Budgets — User Guide](https://docs.aws.amazon.com/cost-management/latest/userguide/budgets-managing-costs.html)
- [AWS Cost Anomaly Detection](https://docs.aws.amazon.com/cost-management/latest/userguide/manage-ad.html)
- [AWS Cost Explorer](https://docs.aws.amazon.com/cost-management/latest/userguide/ce-what-is.html)

## Spring Cloud AWS

- [Spring Cloud AWS — Reference Documentation](https://docs.awspring.io/spring-cloud-aws/docs/current/reference/html/index.html)
- [Spring Cloud AWS — GitHub](https://github.com/awspring/spring-cloud-aws)
- [AWS SDK for Java 2.x — Developer Guide](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/)

## Resiliência e observabilidade

- [Resilience4j — Documentação oficial](https://resilience4j.readme.io/docs)
- [Micrometer — Documentação oficial](https://docs.micrometer.io/micrometer/reference/)
- [springdoc-openapi — Documentação oficial](https://springdoc.org/)

## Ferramentas de desenvolvimento local

- [LocalStack — Documentação](https://docs.localstack.cloud/)
- [draw.io / diagrams.net](https://www.diagrams.net/)

---
**Anterior:** [← 17. Guia de Uso](17-guia-de-uso-swagger.md) | **Próximo:** [19. Catálogo de Produtos AWS →](19-catalogo-produtos-aws.md)
